...
if($result->num_rows>0){
	  ...
	  $text = "";
		while($row = $result->fetch_assoc()){
			if($state=="true"){
               	 if($i==0){
                        $text = substr($row["open_date"],0,8);
						array_push($dates,$text);
					}
					
               if($text == substr($row["open_date"],0,8)){
                   if($x<30) array_push($arr,$row["open_date"].'_'.$row["open_num"]);
				   $x++;
				}else{
                    if($dates[count($dates)-1]!=substr($row["open_date"],0,8))array_push($dates,substr($row["open_date"],0,8));
				}   
			}else{
                   if($_GET["date"] == substr($row["open_date"],0,8)){
                        if($j<30){
